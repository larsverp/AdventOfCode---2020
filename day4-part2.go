...
var totalRightPassports int

func main() {
	data := strings.Split(rawData, "\n\n")
	for _, passportdata := range data {
		passportdata = strings.Replace(passportdata, "\n", " ", -1)
		passport := strings.Split(passportdata, " ")
		checkPassport(passport)
	}
}

func checkPassport(passportData []string) {
	var passport Passport
	for _, passportFields := range passportData {
		data := passportFields[4:]
		switch string(passportFields[0:3]) {
		case "byr":
			if yearInBetween(data, 1920, 2002) {
				passport.byr = true
			}
		case "iyr":
			if yearInBetween(data, 2010, 2020) {
				passport.iyr = true
			}
		case "eyr":
			if yearInBetween(data, 2020, 2030) {
				passport.eyr = true
			}
		case "hgt":
			if validHeight(data) {
		...
		case "ecl":
			if validEyeColor(data) {
				passport.ecl = true
			}
		case "pid":
			if validPassportId(data) {
				passport.pid = true
			}
		}
	}
	if passport.byr == true && passport.iyr == true && passport.eyr == true && passport.hgt == true && passport.hcl == true && passport.ecl == true && passport.pid == true {
		totalRightPassports++
	}
}

func yearInBetween(data string, low int, high int) bool {
	matched, _ := regexp.MatchString(`^[0-9]{4}$`, data)
	if matched {
...
func validHeight(data string) bool {
	matchedInch, _ := regexp.MatchString(`\d+in`, data)
	if matchedInch {
		height, _ := strconv.Atoi(strings.TrimSuffix(data, "in"))
		if height >= 59 && height <= 76 {
			return true
		}
	}
	matchedCm, _ := regexp.MatchString(`\d+cm`, data)
	if matchedCm {
		height, _ := strconv.Atoi(strings.TrimSuffix(data, "cm"))
...
func validEyeColor(color string) bool {
	matched, _ := regexp.MatchString(`^(amb|blu|brn|gry|grn|hzl|oth)$`, color)
	return matched
}

func validPassportId(id string) bool {
	matched, _ := regexp.MatchString(`^[0-9]{9}$`, id)
	return matched
}
