...
var rightPasswords int

func main() {
	for _, value := range passwordList {
		data := strings.Split(value, ":")
		password := strings.TrimSpace(data[1])
		splitOnDash := strings.Split(data[0], "-")
		splitOnSpace := strings.Split(splitOnDash[1], " ")

		index1, _ := strconv.Atoi(splitOnDash[0])
		index2, _ := strconv.Atoi(splitOnSpace[0])
		letter := splitOnSpace[1]

		if string(password[index1-1]) == letter && string(password[index2-1]) != letter {
			rightPasswords++
		}
		if string(password[index2-1]) == letter && string(password[index1-1]) != letter {
			rightPasswords++
		}

	}
	fmt.Printf("There are %d good passwords \n", rightPasswords)
}
